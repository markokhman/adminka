...
<div class="wrapper">
	<!-- Left navigation  al mundo muchahez-->
    <?php 
    ...
    <div class="content" id="container">
        ...
        <div class="stats">
            <ul>                
                <li><a href="#" class="count grey" title=""><?php echo Mark::model()->countByAttributes(array('cid'=>Yii::app()->user->id));?></a><span>отмеченные люди</span></li>
      ...
      chart: {
        ...
        xAxis: {
            ...
            min: <?php if( 3 > round(count($varProposalNames)/1.3)) echo 3; else echo round(count($varProposalNames)/2); ?>
      ...
      trackBorderRadius: 7
        ...
        <div class="fluid">
            <div class="span6">
                <div class="widget"><!-- Pie chart 1 -->
                    ...
                    foreach($certs as $cert)
                    {
                        $rorders = Order::model()->findAllByAttributes(array('cert_id'=>$cert->cert_id, 'status'=>1));
                        foreach($rorders as $rorder)
                        {
                          if(!(in_array($rorder->uid,$ruser_base)))
                          {
                           array_push($ruser_base,$rorder->uid);
                          }
                        }
                    }
                    $rmarks = Mark::model()->findAllByAttributes(array('cid' => Yii::app()->user->id));
                    foreach($rmarks as $rmark)
                    {
                        if(!(in_array($rmark->uid,$ruser_base)))
                          {
                            array_push($ruser_base,$rmark->uid);
                          }
                    }
                
                    $rtoday = new DateTime("now");

                    foreach($ruser_base as $ruser)
                    {
                      $user_entity = User::model()->findByPk($ruser);
                      $rbirthday = new DateTime($user_entity->dateofbirth);
                      $rage = $rbirthday->diff($rtoday);
                      $rnumber = (int)$rage->format('%a');
                      $rnumber = round($rnumber/365);
                      $rgroup = round(($rnumber-12)/3);
                      if($rgroup < 1) $rgroup = 0;
                      if($rgroup > 8) $rgroup = 8;
                      $rdata[$rgroup]++;
                      if ($user_entity->gender == 'мужской') {
                          $varM++;
                        }
                        else
                          $varJ++;
        ...
        <!-- <div class="widget">
          <div class="head"><h5 class="iStats"> Po gruppam</h5>        
          <div id = "rgruppam"> -->
          </div>
          </div>
        </div>
...
<script type="text/javascript">
   var sex = new Highcharts.Chart({
        chart: {
            ...
            xAxis: {
                categories: [' Моложе 15', '15-18', '18-21', '21-24', '24-27', '27-30', '30-33', '33-36', 'Старше 36'],
                title: {
                    text: null
                }
            },
            ...
            series: [{
                ...
                data: [<?php foreach($rdata as $onerdata)
                      {
                        echo $onerdata;
                        echo ',';
                      }
...
<script type="text/javascript">
  ...
  //           series: [{
  //               name: 'People',
  //               data: [<?php echo $user->findSatisfy($grouparam["cert1"],$grouparam["cert2"],$grouparam["check1"],$grouparam["check2"]) ?>,
  //               <?php echo $user->findSatisfy($grouparam["cert2"],$grouparam["cert3"],$grouparam["check2"],$grouparam["check3"])?>,
  //               <?php echo $user->findSatisfy($grouparam["cert3"],10000,$grouparam["check3"],10000)?> ]
