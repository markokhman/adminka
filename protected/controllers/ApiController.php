                ...
                if ($uid != 0) 
                {
                    ...
                    foreach($models as $model)
                    {
                        ...
                        if($user->gender == "женский")
                            ...
                            $gen = 1;
                        $certs = Certificate::model()->findAllBySql('select * from certificate where cid='.$id.' AND (age_min <= '.$age.' AND age_max >= '.$age.') AND (time_begin <= '.$today.' AND time_end >= '.$today.') AND (star_min <= '.$starAmount2.' AND attend_min <= '.$ordersAmount.') AND gender IN (0, '.$gen.') AND (pushType <= 4 and pushType >= 1)');
                        $rows3 = array();
                        /////////////////////////////////////////////////////////////////////////
                        $ce_orders = Order::model()->findAllByAttributes(array('status' => 1, 'uid' => $uid));
                        foreach($ce_orders as $ce_order)
                        {
                            $pareent = $ce_order->cert_id;
                            $pareent +=5;
                            $certs_join = Certificate::model()->findAllByAttributes(array('pushType' => $pareent,'cid' => $id));
                            if($certs_join != null)
                            {
                                foreach($certs_join as $cert_join)
                                {
                                    array_push($certs,$cert_join);
                                }
                            }
                        }
                        ...
                        $img = Images::model()->findByAttributes (array('owner_id'=>$model->cid, 'type'=>'main-client'));
                        if ($img != null)
...
/////////////join
            ...
            foreach($ce_orders as $ce_order)
            {
                $pareent = $ce_order->cert_id;
                $pareent +=5;
                $certs_join = Certificate::model()->findAllByAttributes(array('pushType' => $pareent,));
                if($certs_join != null)
                {
                    foreach($certs_join as $cert_join)
                    {
            ...
            $certss = Certificate::model()->findAllBySql('select * from certificate where (age_min <= '.$age.' AND age_max >= '.$age.') AND (time_begin <= '.$today.' AND time_end >= '.$today.') AND gender IN (0, '.$gen.') AND (pushType <= 4 and pushType >= 1) ORDER BY time_begin DESC');

            foreach($certss as $cert_s)
            {
                array_push($certs,$cert_s);    
            }
/////////////join
            $rows2 = array(); 
            ...
            foreach($certs as $model)
            {
                ...
                $img = Images::model()->findBySql ('SELECT * FROM images WHERE type = "main-client" AND owner_id = '.$model->cid);
                if ($img != null)
                ...
                if ($starAmount2 >= $model->star_min && ($model->star_max == 0 || ($model->star_max != 0 && $starAmount2 <= $model->star_max))) 
                {
                    ...
                    else if ($marksAmount2 >= $model->mark_min && ($model->mark_max == 0 || ($model->mark_max != 0 && $marksAmount2 <= $model->mark_max))) 
                    {
                        if ((($model->number != 0 && $model->number > $markForCert) || $model->number == 0) && $model->numberPerAccount > 0) 
                            {
                                $rows2[] = $certificateForResponse;
            ...
            $arr['result'] = $rows3;
            $arr['status'] = $status;
            $arr['kotik'] = Yii::app()->session['kotik'];
        ...
        public function actionView()
        {
            ...
            else 
            {
                ...
                if($_GET['model'] == 'certificate')
                {
                    ...
                    $visited_user = $_GET['uid'];
                    $visiting_cert = $id;

                    $fact = VisitorData::model()->findAllByAttributes(array('cert_id' => $visiting_cert, 'uid' => $visited_user));
                    if($fact == null)
                    {
                        $fact = new VisitorData();
                        $fact->uid = $visited_user;
                        $fact->cert_id = $visiting_cert;
                        $fact->date = time();
                        if($fact->save())
                        {
                            $cert_info = CertificateInfo::model()->findByAttributes(array('cert_id' => $visiting_cert));
                            if($cert_info == null)
                            {
                                $cert_info = new CertificateInfo();
                                $cert_info->cert_id = $visiting_cert;
                                $cert_info->visited_number = 1;
                                $cert_info->created_date = 0;
                                $cert_info->save();
                            }
                            else
                            {
            ...
            if($_POST['type'] == 'facebook')
            {
                ...
                else
                {
                    foreach($user as $user1)
                    {
                        ...
                        $arr = array();    
                        $arr['kot'] = $friends;    
                        //$arr['kotik'] = $_POST['friend'];;
                        Yii::app()->session['kotik'] = $_POST['friend'];
                        if($user1->save())
                        {
        ...
        public function myw($pouid,$pocid)
        {
            ...
            $orderAmount = $this->attends($uid,$cid,0);
            $markAmount = $this->marks($uid,$cid,2);
            ///hereeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee
            $age = $this->userAge();
            $cert_id = 0;
            $certs = Certificate::model()->findAllBySql('select * from certificate where cid = '.$cid.'  AND (age_min <= '.$age.' AND age_max >= '.$age.') AND (time_begin <= '.$today.' AND time_end >= '.$today.') AND gender IN (0, '.$gen.')  AND ((mark_min <= '.$markAmount.' AND mark_max >= '.$markAmount.')  OR (attend_min <= '.$orderAmount.'  AND attend_max >= '.$orderAmount.')) AND pushType IN (0,0) ORDER BY time_begin DESC');
            
            foreach($certs as $certa)
            {
                $cert_id = $certa->cert_id;
                break;
            }

            $cert = Certificate::model()->findByPk($cert_id);
            ...
            if ($cert == null) {
                ...
                $user_orders = $this->attends($uid, 0, $cert->cert_id);
        ...
        public function actionMark()
       {
            ...
            $arr1 = array();
            $arr1['status'] = $status;    
            $arr1['checkin'] = $this->myw($_POST['uid'],$_POST['cid']);        
            $clientuwka = Client::model()->findByPk($_POST['cid']);
            $arr1['venue_name'] = $clientuwka->title."";
