...
class Certificate extends CActiveRecord
{
	...
	//gives attributes for usual cert
	public function usualize()
	{
		$this->description = '';
		$this->condition = '';
		$this->age_max = 50;
		...
		$this->fb_min = 0;
		$this->number = 0;
		$this->numberPerAccount = 1;
		$this->pushType = 1;
		$this->cid = Yii::app()->user->id;
	}
	public function checkinize()
	{
		$this->age_max = 100;
		$this->age_min = 0;
		$this->attend_max = 100;
...
//eto spartaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa
	...
	// 		//getting array of users for whom the certificate venue is the most favourite place
	// 		$target_client = Client::model()->findByPk($this->cid);
	// 		$need_user = $target_client->getSufficient();//actually this array is users id array
	// 		//getting intersection of users that like place and users who satisfy some part of criteria
	// 		if($this->pushType > 4)
	// 		{
	// 			$oorders = Order::model()->findAllByAttributes(array('cert_id'=>$this->pushType-5,'status' => 1));
	// 			$order_users = array();
	// 			foreach($oorders as $oorder)
	// 		 	{	
	// 				array_push($oorder->uid,$order_users);
	// 		   	}
			
	// 		 	foreach($users as $key=>$value)
	// 		 	{
	// 				if(!(in_array($value->uid, $need_user)))
	// 				{
	// 					unset($users[$key]);
	// 				}
	// 				else if(!(in_array($value->uid, $order_users)))
	// 				{
	// 					unset($users[$key]);	
	// 				}
	// 			}
    ...
    public function sendPushNotification($uid)
	{
