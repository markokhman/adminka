...
/////////////join
        ...
        public function myw($pouid,$pocid)
        {
            ...
            $age = $this->userAge();
            $cert_id = 0;
            $certs = Certificate::model()->findAllBySql('select * from certificate where (age_min <= '.$age.' AND age_max >= '.$age.') AND (time_begin <= '.$today.' AND time_end >= '.$today.') AND gender IN (0, '.$gen.')  AND ((mark_min <= '.$markAmount.' AND mark_max >= '.$markAmount.')  OR (attend_min <= '.$orderAmount.'  AND attend_max >= '.$orderAmount.')) AND pushType IN (0,0) ORDER BY time_begin DESC');
